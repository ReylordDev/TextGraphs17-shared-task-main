...
def wikidata_ids2names(ids_str, wikidata_id2name, sep="\\"):
    ...
    names = [
        wikidata_id2name[w_id.strip()]
        if wikidata_id2name.get(w_id.strip()) is not None
        else NONE_WIKIDATA_IDS.get(w_id.strip(), "NONE")
...
def parse_args():
    ...
    parser.add_argument(
        "--output_train_path", type=str, required=False, default="../data/tsv/train.tsv"
    )
    parser.add_argument(
        "--output_dev_path", type=str, required=False, default="../data/tsv/dev.tsv"
    )
    parser.add_argument(
        "--output_test_path", type=str, required=False, default="../data/tsv/test.tsv"
    )
...
def main(args):
    ...
    train_df[
        [
            ...
            "answerEntityId",
            "questionEntityId",
            "groundTruthAnswerEntityId",
            "graph",
            "correct",
        ]
    ].to_csv(output_train_path, index=False, sep="\t")
    ...
    ].to_csv(output_dev_path, index=False, sep="\t")
    test_df[
        [
            "sample_id",
            "subset",
